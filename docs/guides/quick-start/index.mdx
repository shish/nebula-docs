...
By default, the CA certificate will expire after 1 year, and client certificates will expire when the CA certificate
does. You can change the duration of both types of certificate with the `-duration` flag, though note that a client
certificate can never last longer than the CA one.
